...
from DeepNovelARG.dataset import (
    obtain_labels,
    obtain_dataset_wordvectors,
    obtain_dataset_alignments,
    obtain_test_labels,
)
...
@click.command()
@click.option(
    "--inputdir", required=True, help="input directory with the output from fasta2vec"
)
@click.option(
    "--outdir",
    default="",
    required=True,
    help="output directory where to store the model",
)
@click.option(
    "--epoch",
    default=10,
    required=False,
    help="number of epochs to run the model [default 10]",
)
@click.option(
    "--batch",
    default=32,
    required=False,
    help="batch size for using during training [default 32]",
)
@click.option(
    "--maxlen-conv",
    default=1500,
    required=False,
    help="max sequence length to consider for convolutional network [default 1500]",
)
@click.option(
    "--prefix",
    default="model",
    required=False,
    help="prefix used during training for tensorboard. Useful for keeping track of different sessions",
)
def train(inputdir, outdir, epoch, batch, maxlen_conv, prefix):
    ...
    # tensorboard
    name = "deepARG+_" + prefix + "_" + str(time())
    log.info("starting TensorBoard")
    tensorboard = TensorBoard(log_dir=outdir + "/logs/" + name)

    # Model Checkpoint
    ckpt_file = outdir + prefix + ".{epoch:03d}.hdf5"
    ...
    json.dump(
        ...
        open(outdir + prefix + ".parameters.json", "w"),
    )
